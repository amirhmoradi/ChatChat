...
export async function sendClaudeStreamMessages(model: string, message: string, apiKey: string, claudeAPITemperature: number) {
    if (!apiKey) {
    ...
    const response = await fetch('https://claude.azure-api.net/complete', {
        method: 'POST',
        headers: {
